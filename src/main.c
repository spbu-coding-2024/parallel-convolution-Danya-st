...
void tile_convolution(unsigned char *src, unsigned char *result, int w, int h,
                      int TILE_W, int TILE_H, Filter *f) {
...
int main(int argc, char *argv[]) {
  if (argc < 3) {
    fprintf(
        stderr,
        "usage: %s <input.png> <output> [--filter=name] [--parallel=mode]\n",
        argv[0]);
  ...
  case MODE_PIXEL:
    ...
    break;
  case MODE_ROW:
    row_convolution(src, result, w, h, &filter);
    break;
  case MODE_COL:
    column_convolution(src, result, w, h, &filter);
    break;
  case MODE_TILE:
    tile_convolution(src, result, w, h, tile_w, tile_h, &filter);
    break;
  default:
